 ...
 * through the building, and that sentence is what justifies the animation —
 * without one it would be deleted.
 *
 * NO PIN. The holding-in-place is CSS `position: sticky`, exactly as the
 * floor-plan stack does it, and GSAP only moves the track sideways.
 *
 * That is not a stylistic preference, it is a bug fix. ScrollTrigger's pin
 * locks the pinned element to the width it measured at pin time — it writes
 * `width` and `max-width` in pixels inline — and this section clips its own
 * overflow, so the track was being cut off at whatever that stale number
 * happened to be. Anything that changed the layout after the measurement
 * (a resize, a late font, a scrollbar appearing) left a hard vertical edge
 * partway across the screen with bare floor beyond it and four cards
 * unreachable behind it. A sticky element is never taken out of flow, so it
 * has no measured width to go stale: it is simply always as wide as its
 * parent. The reference has zero pins across 64 ScrollTriggers, and this is
 * the kind of reason why.
 *
 ...
 * floor at 13%. You are standing inside now; the floor says so.
 */
...
export function AmenityPan() {
  ...
  useEffect(() => {
    ...
    // A phone's URL bar showing and hiding changes viewport height mid-scroll
    // and would otherwise force a refresh under the reader.
    ...
    mm.add("all", () => {
      ...
      // edge. Measured against the track's own width — `window.innerWidth`
      // includes the vertical scrollbar and would leave the pan a scrollbar
      // short of the final card.
      const travel = () => Math.max(0, el.scrollWidth - el.clientWidth);

      // How much page scroll that costs the reader.
      //
      // Slightly LONGER than the travel, never shorter. Squeezing 2010px of
      // track into 1800px of scroll made the track move 1.12px per pixel
      // scrolled, and with scrub lag on top an ordinary flick of the wheel
      // outran it entirely. At 1.15x the track has slack behind the scroll,
      // so it arrives rather than chases. The ceiling is the 2.5 viewport
      // heights the brief asks for: horizontal scroll is the one place
      // readers most often feel trapped.
      const scrollLength = () =>
        Math.min(travel() * 1.15, window.innerHeight * 2.5);

      /**
       * The section's own height IS the scroll length. The sticky viewport
       * inside it stays put until the section has been scrolled through, so
       * how long the walk lasts is simply how tall this is.
       *
       * This has to be re-derived on resize and not just at setup. The cards
       * are sized in `vw`, so a wider window means a longer track: at 1440
       * the walk is 2010px, at 1920 it is 2605px. Leave the height at the
       * old figure and the sticky viewport releases before the track has
       * finished moving, which strands the last cards off-screen — the same
       * class of staleness the pin used to have, arriving by a different
       * door.
       */
      const setHeight = () => {
        container.style.setProperty("--walk-scroll", `${scrollLength()}px`);
      };
      setHeight();

      // ScrollTrigger refreshes on resize and that fires `onRefreshInit`
      // below, which is the primary path. This observer is the backstop: it
      // watches the thing that actually determines the answer — the track's
      // own width — so the height stays correct even if a refresh is missed
      // or debounced away. Cheap, and it removes the need to trust a
      // refresh cycle for something a wrong value makes unreadable.
      ...
      gsap.to(el, {
        ...
        scrollTrigger: {
          trigger: container,
          start: "top top",
          // The section is exactly `100svh + scrollLength` tall, so its
          // bottom edge reaches the viewport bottom precisely when the walk
          // is over. No `+=` arithmetic, and nothing to keep in sync.
          end: "bottom bottom",
          scrub: 0.5,
          invalidateOnRefresh: true,
          // Re-derive the height before ScrollTrigger measures anything.
          // Track width is driven by `vw`, so this depends only on the
          // viewport and can never feed back into its own measurement.
          onRefreshInit: setHeight,
          onUpdate: (self) => {
            // The rail and the counter are the reader's only way to know how
            // much walk is left. Written straight to the DOM — re-rendering
            // React on every scroll frame is how this starts dropping frames.
            if (rail.current) {
      ...
      return () => {
        ...
        container.style.removeProperty("--walk-scroll");
      };
    });
  ...
  return (
    ...
    >
      {/* The sticky viewport. In list mode this is an ordinary block and
          does nothing; in pan mode it is what holds the walk in place while
          the section scrolls past behind it. */}
      <div className="walk-viewport">
        <MagneticLabel label="Walk through" className="h-full">
          <div ref={track} className="walk-track">
