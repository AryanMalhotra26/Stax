...
export default function RegisterPage() {
  return (
    <>
      <main id="main">
        <section className="relative flex h-dvh min-h-[44rem] flex-col overflow-hidden bg-charcoal">
          <Render
          ...
          <header className="relative shrink-0">
            <div className="container-stax flex h-18 items-center justify-between md:h-20">
              <Link href="/" aria-label="Stax — home">
                <Logo className="h-6 w-auto text-white md:h-7" />
              </Link>
              <Link
                href="/residences"
                className="text-eyebrow uppercase text-white/70 transition-colors hover:text-white"
              >
                Floor plans
              </Link>
            </div>
          ...
          <div className="container-stax relative flex flex-1 items-center py-6">
            <div className="grid w-full items-center gap-10 lg:grid-cols-[1fr_26rem] lg:gap-16 xl:grid-cols-[1fr_28rem]">
              ...
              <div className="hidden sm:block">
                <p className="animate-rise stagger-1 mb-6 inline-flex w-fit items-center gap-2.5 bg-brick px-3.5 py-2 text-eyebrow uppercase whitespace-nowrap text-white">
                  <span className="h-1.5 w-1.5 shrink-0 bg-white" aria-hidden="true" />
                  Priority list · {SITE.facts.occupancyShort}
                </p>

                <h1 className="animate-rise stagger-2 text-h1 uppercase text-white">
                  Get in
                  <br />
                  first.
                </h1>

                <p className="animate-rise stagger-3 mt-6 max-w-md text-lead text-white/75">
                  Fifteen seconds, no obligation. Plans, pricing and lease dates
                  go to this list before they go anywhere else.
                </p>

                <dl className="animate-rise stagger-4 mt-9 hidden max-w-md grid-cols-3 gap-6 border-t border-white/20 pt-7 lg:grid">
              ...
              <div className="animate-rise stagger-3 w-full bg-ink/95 p-6 backdrop-blur-md md:p-8">
                <p className="mb-6 text-eyebrow uppercase text-white/45 sm:hidden">
                  Priority list · {SITE.facts.occupancyShort}
                </p>
                <h2 className="mb-6 text-h3 text-white sm:hidden">Get in first.</h2>
                ...
                <div className="mt-6 border-t border-white/15 pt-5">
                  <p className="text-sm text-white/55">
                    ...
                    <a
                      href={SITE.phoneHref}
                      className="font-semibold text-white underline underline-offset-4 hover:text-brick"
                    >
                      {SITE.phone}
                    </a>
                  </p>
...
function Mini({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <dd className="text-h3 tnum text-white">{value}</dd>
      <dt className="mt-1.5 text-eyebrow uppercase text-white/45">{label}</dt>
    </div>
  );
}
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <li>
      ...
      <h2 className="text-h3 mt-5 text-white">{title}</h2>
